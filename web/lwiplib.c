...
#if LWIP_SSDP || LWIP_SDDP

void LwIP_DHCP_task(void *arg);
...
//*****************************************************************************
#if !NO_SYS
#define STACKSIZE_LWIPINTTASK   1024
#endif
...
lwIPInterruptTask(void *pvArg)
{
    ...
    while(1)
    {
        ...
        while(xQueueReceive(g_pInterrupt, &pvArg, portMAX_DELAY) != pdPASS)
        {
				...
				//s_printf("will call tivaif_interrupt\r\n");
...
lwIPLinkDetect(void)
{
    ...
    g_bLinkActive = bHaveLink;
		...
		/*when in static ip mode, no need do the below handler*/
...
lwIPPrivateInit(void *pvArg)
{
 ...
 // SsdpInit(&g_sNetIF);
	...
	tcpip_timeout(1000, LwIP_DHCP_task, NULL);
#endif


s_printf("lwIPPrivateInit init done\r\n");
         ...
         uint32_t ui32NetMask, uint32_t ui32GWAddr, uint32_t ui32IPMode)
{
    ...
    if((EMAC_PHY_CONFIG & EMAC_PHY_TYPE_MASK) == EMAC_PHY_TYPE_INTERNAL)
    {
        ...
        else
        {
            ...
            while(1)
            {
							 s_printf("Internal PHY is not present on this part\r\n");
            }
        }
    }
...
ip_addr_t *get_cur_ip(void)
{
	  return &g_sNetIF.ip_addr;
}
...
extern void update_ip_menu(void);
...
void LwIP_DHCP_task(void *arg)
{
  ...
  unsigned char static_ip = arg? true:false;
  if((!ip_addr_isany(&g_sNetIF.ip_addr)&&
     !ip_addr_cmp(&g_sNetIF.ip_addr,&ip)) || (++static_ip_timeout > 60 && static_ip)){
       if(!static_ip){   
         LWIP_DEBUGF(DHCP_DEBUG, ("Lwip dhcp task: static_ip %d\n"));
         ip_addr_set(&ip,&g_sNetIF.ip_addr);
         SetLocalhost(&g_sNetIF.ip_addr);
         SetNetMask(&g_sNetIF.netmask);
         SetGateway(&g_sNetIF.gw);
				 update_ip_menu();
       }else{
         ip_addr_set(&ip,Localhost());
         netif_set_addr(&g_sNetIF, Localhost(),GetNetMask(),GetGateway());
         dhcp_stop(&g_sNetIF);
         return;
       }
  }
